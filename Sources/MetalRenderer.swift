...
class MetalRenderer {
    ...
    func captureMouse() {
        ...
        keyboardEventMonitor = NSEvent.addLocalMonitorForEvents(matching: [.keyDown, .keyUp, .flagsChanged]) { [weak self] event in
            ...
            if windowsVK != 0 {
                ...
                if event.type == .keyDown {
                    ...
                    if event.keyCode == 56 || event.keyCode == 60 { // Shift
                        keyAction = currentFlags.contains(.shift) ? Int8(KEY_ACTION_DOWN) : Int8(KEY_ACTION_UP)
                    } else if event.keyCode == 59 || event.keyCode == 62 { // Control
                        keyAction = currentFlags.contains(.control) ? Int8(KEY_ACTION_DOWN) : Int8(KEY_ACTION_UP)
                    } else if event.keyCode == 58 || event.keyCode == 61 { // Option/Alt
                        keyAction = currentFlags.contains(.option) ? Int8(KEY_ACTION_DOWN) : Int8(KEY_ACTION_UP)
                    } else if event.keyCode == 55 || event.keyCode == 54 { // Command
                        keyAction = currentFlags.contains(.command) ? Int8(KEY_ACTION_DOWN) : Int8(KEY_ACTION_UP)
                    } else {
                        return event
                    }
                }
        ...
        print("Mouse captured - Press Shift+Ctrl+Option+M to toggle capture, Shift+Ctrl+Option+Q to quit")
    }
